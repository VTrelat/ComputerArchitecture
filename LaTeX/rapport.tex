...
Il restera donc au plus 7 entrées à traiter après la boucle.\\
...
\end{itemize}
L'opération correspondant à \texttt{sum\_comp} est coûteuse et correspond à la commande \texttt{movd eax, xmm0}.
Enfin, les potentiels restes sont traités par la suite (de $\frac{n-4}{4}+1$ à $n$) et la somme est effectuée en même temps.
% (n - 4)/4 + 1
% n - 1 <= 8 ?
%     true : edx = 0, eax = 0
%     false:  xmm0 = 0,0,0,0 -> s
%             xmm2 = 4,4,4,4 -> w
%             ecx = 0 -> i
%             xmm1 = 0,1,2,3 -> cur
%             ecx = 1 -> i
