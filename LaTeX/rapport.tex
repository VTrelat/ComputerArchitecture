...
Autrement dit, on traitera les entrées modulo 4 et décalées de 4, car le compilateur considère que cette valeur de seuil est la plus rentable\footnote{Pour un seuil $s_k = 4k$, on aurait "$\texttt{eax} = \frac{n-s_k}{4}+1$" et "\texttt{cmp ecx $s_k+4$}"}.
...
b. \begin{itemize}
    \item[\texttt{pcmpgtd}] : effectue une comparaison signée en SIMD avec la condition \textit{greater than} pour \textit{double word}.\\
    \item[\texttt{pand}] : opérateur et logique pour des registres XMM (128 bits).\\
    \item[\texttt{pandn}] : opérateur de non et logique pour des registres XMM (128 bits). $\texttt{pandn $p$ $q$} \equiv p \leftarrow \neg p \wedge q$.\\ 
\end{itemize}

\begin{lstlisting}[style=customasm]
; On dispose du registre xmm2 = 1,1,1,1.
pcmpgtd xmm1, xmm0
xmm0 = xmm1
pandn xmm1, xmm2
por xmm0, xmm1
