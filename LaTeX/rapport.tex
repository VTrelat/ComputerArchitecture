...
\begin{framed}
    ...
    Lorsqu'on fait des accès séquentiels, on charge des blocs en cache, et on fait une suite d'accès en cache, ce qui est rapide.
...
\end{lstlisting}
Le fait que la somme des tailles diffère de la taille de la somme (de la structure) provient de la compilation : le compilateur fait de l'alignement (\textit{padding}) sur des multiples de 8 afin qu'un élément se trouve toujours en début de bloc.
...
\noindent\begin{minipage}{.45\textwidth}
    \begin{lstlisting}[caption=Optimisation O1, style=customasm]
    main:
        sub	rsp, 8
        mov	r9d, 0
        mov	r8d, 0
...
\begin{minipage}{.45\textwidth}
    \begin{lstlisting}[caption=Optimisation O2, style=customasm]
    main:
        sub	rsp, 8
        xor	r9d, r9d
        xor	r8d, r8d
...
\end{minipage}
\begin{itemize}
    \item[$\bullet$] Première différence entre O0 et O1: le code assembleur utilise plus de registres et ne fait pas d'accès en RAM.\\
    \item[$\bullet$] Deuxième différence : au niveau O2, on utilise plutôt des instructions comme \texttt{xor rax, rax} pour remplacer les instructions \texttt{mov rax, 0} qui sont plus rapides et prennent moins de place en mémoire.\\
    \item[$\bullet$] Troisième différence : le niveau O2 déplace des instructions afin de réduire les bulles.
    Par exemple, la ligne 11 du code O2 correspond à la ligne 27 du code O1.\\    
\end{itemize}
...
\begin{lstlisting}[style=customasm, firstnumber=43]
## %bb.6:
	test	r9b, 1
	je	LBB0_8
LBB0_7:
...
Tout d'abord, on observe que la vectorisation se fait sur des vecteurs de taille 8 (L.19-22), et donc les données sont espacées en mémoire de 2 octets (16 bits\footnote{Les registres xmm peuvent stocker 128 bits.}).
La commande utilisée pour trouver le min est \texttt{pminsd}.

\restoregeometry

\subsection{Exercice 4}
a. On dispose de 1024\footnote{$\frac{64~\text{ko}}{64~\text{o par ligne}}$} blocs et $\frac{64*1024}{64} = 1024$ sous-caches contenant chacun un bloc.
On sait également que les entiers sont codés sur 4 octets, un bloc peut donc contenir 16 entiers. On peut donc stocker en cache $1024 \times 16 = 16384$ entiers simultanément. De plus, on accède aux données dans le tableau (ou dans chaque sous-tableau) de manière consécutive.\\
...
c. \begin{itemize}
    ...
    \begin{algorithm}[H]
        \SetAlgoLined
        \KwData{An image of size $\texttt{LARGEUR} \times \texttt{HAUTEUR}$}
        \ForAll{$i \in [\![0, \dots, \left\lfloor \frac{\texttt{LARGEUR}\times\texttt{HAUTEUR}}{16} \right\rfloor]\!]$}{
            $v_R := \texttt{image}[i:i+15]$\;
            $v_G := \texttt{image}[i+\texttt{LARGEUR}\times\texttt{HAUTEUR}:i+\texttt{LARGEUR}\times\texttt{HAUTEUR}+15]$\;
            $v_B := \texttt{image}[i+2\times\texttt{LARGEUR}\times\texttt{HAUTEUR}:i+2\times\texttt{LARGEUR}\times\texttt{HAUTEUR}+15]$\;
            $w := \displaystyle\frac{v_R + v_G + v_B}{3}$\;
            $\texttt{dest}[i:i+15] = w$\;
            $\texttt{dest}[i+\texttt{LARGEUR}\times\texttt{HAUTEUR}:i+\texttt{LARGEUR}\times\texttt{HAUTEUR}+15] = w$\;
            $\texttt{dest}[i+2\times\texttt{LARGEUR}\times\texttt{HAUTEUR}:i+2\times\texttt{LARGEUR}\times\texttt{HAUTEUR}+15] = w$\;
        }
