...
\begin{equation*}
    t_{\text{seq}} \colon n \mapsto \xi_\text{seq} n
\end{equation*}

On constate ainsi que le temps mis pour faire un accès mémoire séquentiel ou aléatoire est constant!\footnote{On a obtenu plus haut que le temps de la mesure augmente linéairement avec le nombre d'accès mémoire. En moyennant, on obtient bien un temps constant par mesure.}
% faire ex8 chap 3 en plus
\end{document}